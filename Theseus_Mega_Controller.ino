...
void moveRobot(dirX, powerX, dirY, powerY)
{
...
void loop() {
  ...
  if (PS4.connected()) {
    ...
    // Assuming positive direction is right
    
    if ((leftDeadzoneX == -1 || leftDeadzoneX == 1) && leftDeadzoneY == 0)
    {
      moveWheels(leftStickX, 0);
    }

    else if (leftDeadzoneX == 0 && (leftDeadzoneY == -1 || leftDeadzoneX == 1))
    {
      moveLaterally(0, leftStickY);
    }
